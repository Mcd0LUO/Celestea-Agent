...
| R3 | 测试夹具仍按两个 id 查找 | 合并会改 id 面：`tests/w795-optimistic-grants.test.ts`、`w871-shell-anchor-dom.test.ts`、`w847-inline-attach-sl-end-dom.test.ts` 等需要同步；**保留旧 id 作为别名**是禁止的（会留下两个入口的假象） |
