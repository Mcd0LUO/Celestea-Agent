...
> 第 2 版改动：① 补入 DSH 官方 cookbook 的**渐进式披露配方**（§1.7b–d），纠正第 1 版把 pi-ai 兼容门误读为「DSH 不做渐进披露」；② 缓存章节增加**本轮独立复核探针 A1–A7**（§3.3）；③ 修正账本快照的时点标注。
...
| D3 | 缓存（本设计的关键约束） | 11 个真实工具 ≈ **4329 prompt token**；请求序列化顺序 = **system → tools → messages**。**tools 数组的任何变化都会让它之后的 token（含整段历史）从变动点起失效。**本轮独立复核（§3.3）：稳定态 hit **97.5%**；**尾部追加 1 工具 → 67.7%**（−2432 cached token）；**首工具移尾 → 23.3%**（−6528）；**中段删 1 工具 → 37.6%**（−5376）；改回原样立刻恢复 97.5%；改动后下一次请求 97.7%（一次性重热）。**结论：动态披露必须只在 turn 边界、单调只增、新增追加在数组尾部、系统提示词绝不随披露变。** |
