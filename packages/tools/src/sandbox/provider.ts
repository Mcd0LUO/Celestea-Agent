...
export function selectSandboxDetailed(options: SelectOptions = {}): SandboxSelection {
  ...
  if (probe.bwrapUsable && probe.bwrapPath !== null) {
    const sandbox = new BwrapSandbox(config, {
      ...
      run: bwrapOptionsFromEnv(env),
      rlimits: rlimitsEnabled(env),
    });
    return { sandbox, provider: BWRAP_PROVIDER, degraded: false, reason: null, mode };
  }
  ...
  return { sandbox: new UserspaceSandbox(config), provider: "userspace", degraded: true, reason, mode };
}
...
/** bwrap knobs from the contract env vocabulary. */
export function bwrapOptionsFromEnv(env: NodeJS.ProcessEnv = process.env): BwrapOptions {
  return {
    shareNet: envFlag(env[ENV_SANDBOX_NET], DEFAULT_BWRAP_OPTIONS.shareNet),
