...
/**
 * W847 A1 · katex 0.16.47→0.18.7：MathML 白名单按真实产物锁定
 *
 * sanitize.ts 的 MathML 白名单必须恰好覆盖 KaTeX output:'mathml' 的真实产物：
 *   · 覆盖：真实 katex 0.18.7 在下方语料上产出的每个标签，过 sanitize 后仍在
 *     （标签被解包 = 丢结构）；
 *   · 不多放：KaTeX 不会产出的 MathML 标签，过 sanitize 后必须消失 —— 多放一个就红。
 *
 * 语料类别：分数/根号/上下标/矩阵（含 hline/vline）/left-right/重音/上下划线/\text/
 * \operatorname/颜色/实体/多行/token 间距/取消线。与 A1 重测脚本同口径。
 */
...
const CORPUS: Record<string, string> = {
  ...
  accents: '\\hat{x} \\bar{x} \\vec{x} \\dot{x} \\ddot{x} \\tilde{x} \\widehat{abc} \\overrightarrow{AB}',
  lines: '\\underline{x} \\overline{x} \\overbrace{x} \\underbrace{x}',
  text: '\\text{hello} \\operatorname{sin} \\operatorname*{argmax}',
...
function tagsOf(html: string): Set<string> {
  ...
  for (const m of html.matchAll(/<([a-zA-Z][\w:-]*)/g)) { const t = m[1]; if (t !== undefined) out.add(t.toLowerCase()); }
  return out;
}

describe('W847 A1 · MathML 白名单覆盖真实 katex 产物', () => {
  it('真实 katex 0.18.7 语料产出的每个标签，过 sanitize 后仍在（不被解包）', async () => {
    const k = await katex;
    ...
    for (const [name, tex] of Object.entries(CORPUS)) {
      for (const display of [false, true]) {
        ...
        const kept = tagsOf(sanMod.sanitizeHtml(raw));
        for (const tag of tagsOf(raw)) {
          if (!kept.has(tag)) missing.push(name + '/' + String(display) + '/' + tag);
        }
      }
    }
  ...
  it('mpadded / munder / mspace 的实测属性保留（含 +6pt 这类带正号长度）', () => {
  ...
  it('新增属性的非法值仍被拒（不因补齐实测而放宽）', () => {
    const bad = sanMod.sanitizeHtml(
      '<math><mpadded width="url(javascript:alert(1))" lspace="javascript:x" voffset="red"><mi>x</mi></mpadded>' +
        '<munder accentunder="evil"><mi>y</mi><mo>_</mo></munder></math>',
    );
