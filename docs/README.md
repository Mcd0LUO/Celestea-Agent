...
| [`modes-standard-vs-execution.md`](./modes-standard-vs-execution.md) | 设计 | 特性设计：**会话双模式**（标准模式 / 执行模式，即 DSH PTC 对应物）的目标契约、分期与可机械检验的验收标准 | 本文；PTC 语义来源见 `/src/celestea_harness/docs/archive/dsh-ptc-mode-eval.md` |
...
整理时（2026-09-11）`docs/` 下只有上表 6 份；**新增文档必须在上表登记**（在途：`docs/feature-session-context.md`，会话上下文接口设计，尚未提交，落地后请登记）。
契约类真源不在 `docs/`，而在
[`../contracts/`](../contracts/)（`endpoints.json` 43 端点、`sse-events.json`、`tools.json`、`data-files/`）——
