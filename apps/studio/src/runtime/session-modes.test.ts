...
describe("W729 per-session mode prompts (real engine, one process)", () => {
  ...
  it("M7 (P1, supersedes P0 invariant ②): execution folds the direct face, standard does not", async () => {
    ...
    expect(faceOf("sample-ws/std")).toEqual(["ask_user_question", "browser_act", "browser_open", "forget", "http_request", "list_dir", "load_skill", "process_control", "read_file", "read_image", "remember", "run_code", "run_shell", "send_message", "spawn_worker", "stop_worker", "worker_status", "write_file"]);
    expect(faceOf("sample-ws/exec")).toEqual(["browser_act", "browser_open", "forget", "http_request", "load_skill", "process_control", "remember", "run_code", "send_message", "spawn_worker", "stop_worker", "worker_status"]);
  });
