...
| `feature-*.md` / `iteration-*.md` | 某个特性的**设计依据与验收标准**（为什么这么设计） | 当前行为的复述（→ ARCHITECTURE） |
| `docs/research/` / `migration/` | 调研与迁移留痕（历史，不逐篇登记） | 现行口径 |
...
**这些不靠自觉**：`tests/doc-conventions.test.ts` 机械检查三条 —— ① 登记与地图链接 ② 状态存在且与地图同类 ③ 相对链接与锚点；违反即 `pnpm check` 红。
