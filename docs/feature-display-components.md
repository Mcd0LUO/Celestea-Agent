...
alerts（`> [!NOTE]`）、脚注、定义列表、TOC。全部走 `marked` 扩展 + 现有白名单标签，仍不动净化器。

### P2 —— 数据类与媒体（需单独立项）

JSON/JSONL 树、CSV->表格、图片灯箱。**图表 / Mermaid / 任意 HTML 预览不在本设计范围内**：
它们需要放宽 `DROP_WITH_CONTENT`（`svg`/`canvas`/`iframe`），那是**安全模型变更**，
必须单独评审，不混在「加显示能力」里顺带做。
