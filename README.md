...
| macOS（Intel） | `celestea-macos-x86_64.tar.gz` | x86_64 |
...
- CI：每次 push / PR 自动跑 `cargo test --workspace`；打 `v*` tag 自动构建三平台预编译产物并发布到 GitHub Releases（也可在 Actions 页手动 `workflow_dispatch` 触发）。
