...
const EXECUTION_FACE = ["browser_act", "browser_open", "forget", "http_request", "load_skill", "process_control", "remember", "run_code", "send_message", "spawn_worker", "stop_worker", "update_tasks", "worker_status"];
const STANDARD_FACE = ["ask_user_question", "browser_act", "browser_open", "forget", "http_request", "list_dir", "load_skill", "process_control", "read_file", "read_image", "remember", "run_code", "run_shell", "send_message", "spawn_worker", "stop_worker", "update_tasks", "worker_status", "write_file"];
...
describe("W791 P1 mode tool face (real engine)", () => {
  ...
  it("M10: the switch rewrites session.json, recomposes at the NEXT turn and spares other sessions", async () => {
    ...
    await activate(h, "sample-ws/plain");
    await activate(h, "sample-ws/std");
    expect((await getJson(h.app, "/api/tools?session=sample-ws%2Fplain")).body["tools"]).toHaveLength(19);
