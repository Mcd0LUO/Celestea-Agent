...
const STANDARD_FACE = ["ask_user_question", "http_request", "list_dir", "process_control", "read_file", "run_code", "run_shell", "session_send_message", "spawn_worker", "worker_status", "write_file"];
...
describe("W791 P1 mode tool face (real engine)", () => {
  ...
  it("M10: the switch rewrites session.json, recomposes at the NEXT turn and spares other sessions", async () => {
    ...
    await activate(h, "sample-ws/plain");
    await activate(h, "sample-ws/std");
    expect((await getJson(h.app, "/api/tools?session=sample-ws%2Fplain")).body["tools"]).toHaveLength(11);
  ...
  it("M10: the switch keeps title/model/prompt and rejects an unknown mode or session", async () => {
    ...
    const before = readFileSync(join(h.workspace, id.slice(id.indexOf("/") + 1), "session.json"), "utf8");
    ...
    const after = JSON.parse(readFileSync(join(h.workspace, id.slice(id.indexOf("/") + 1), "session.json"), "utf8")) as Record<string, unknown>;
