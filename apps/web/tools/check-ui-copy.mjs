 ...
 *
 * 规则一（W517 §7.5）：**含中文的字符串字面量**（= 会渲染给用户看的文案）里，
 *   禁止出现实现细节词：
 *   /api/ · SSE · HTTP␠ · 409 · jsonl · 热调 · 后端 · 前端 · 接口 ·
 *   modified · ev: · lagged · available.models · cache_read · prompt_tokens
 *
 * 规则二（W795）：同一批字面量里也禁止出现**进度占位文案**
 *   （加载中 / 加载清单 / 正在加载 / 切换中 / 提交中 / 正在提交 / 正在读取 /
 *    正在应用 / 正在授予 / 正在撤销 / 读取中 / 进行中 + 空白）。
 *   为什么是门禁而不是口头约定：W795 的裁决是「能立即推出终态的交互一律先画终态，
 *   请求后台跑、失败回滚并说明原因」，占位文案一旦回流就说明这条口径被推翻。
 *   之所以能直接扫字面量：注释不是字面量 —— 注释里描述历史行为（如「不再显示
 *   加载清单中…」）不会被误伤，只有真要渲染给用户的字符串才会命中。
 *   注意 `turn 进行中，无法切换模式`（服务端冻结文案的原样回显）刻意**不算**占位，
 *   所以「进行中」这一条要求后面跟空白（进度形态写的是「进行中 i/n」）。
 *
 * 为什么只查「含中文」的字面量：
...
console.log(
  '✓ UI 文案门禁通过：src/**/*.ts 与 index.html 的用户可见中文文案无实现细节词、无进度占位文案。',
);
